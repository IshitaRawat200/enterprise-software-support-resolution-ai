...
async def view_knowledge_document(
    ...
    if not document.file_data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=(
                "Original uploaded file is not "
                "available for this document."
            ),
        )

    media_type = _media_type_for_document(
        document
    )
...
async def upload_knowledge_document(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=(
                "Unable to ingest the knowledge-base "
                "document."
            ),
        ) from exc
    ...
    try:
        ...
        logger.exception(
            "RAG cache invalidation failed after "
            "document upload document_id=%s",
            document_id,
        )
...
async def delete_knowledge_document(
    ...
    try:
        ...
        logger.exception(
            "Knowledge-base deletion failed "
            "document_id=%s",
            document_id,
        )
    ...
    try:
        ...
        logger.exception(
            "RAG cache invalidation failed after "
            "document deletion document_id=%s",
            document_id,
        )
